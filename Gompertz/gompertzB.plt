...
set xtics auto #("0" 0,"" 25, "50" 50, "" 75, "100" 100) #30, "40" 40)#, "50" 50)#, "60" 60)
set ytics ("0" 0, "" 1.25, "2.5" 2.5, "" 3.75, "5" 5.0)#, "" 75, "100" 100) #("{/Symbol K/2}" 50, "{/Symbol K}" 100)#("10^3" 1000) #, "" 0.05, "1" 0.1,  "" 0.15, "2" 0.2,  "" 0.25, "3" 0.3,  "" 0.35, "4" 0.4)
...
f1(x) = -(exp(-l1*x)*log(x0/K))
f2(x) = -(exp(-l2*x)*log(x0/K))
f3(x) = -(exp(-l3*x)*log(x0/K))
#fb = log(K/x0)
#f1(x) = (exp(-l1*x)*log(x0))
#f2(x) = (exp(-l2*x)*log(K/x0))
#f3(x) = (exp(-l3*x)*log(K/x0))
##f4(x) = c0*exp(a4*x)
#f5(x) = c0*exp(a5*x)
