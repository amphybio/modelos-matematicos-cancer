...
set ytics ("0" 0,"" 25, "50" 50, "" 75, "100" 100) #("{/Symbol K/2}" 50, "{/Symbol K}" 100)#("10^3" 1000) #, "" 0.05, "1" 0.1,  "" 0.15, "2" 0.2,  "" 0.25, "3" 0.3,  "" 0.35, "4" 0.4)
...
f1(x) = (x0*K)/((K-x0)*exp(-l1*x)+x0)
f2(x) = (x0*K)/((K-x0)*exp(-l2*x)+x0)
f3(x) = (x0*K)/((K-x0)*exp(-l3*x)+x0)
#f4(x) = c0*exp(a4*x)
#f5(x) = c0*exp(a5*x)
